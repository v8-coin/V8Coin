...
const char * const BITCOIN_CONF_FILENAME = "v8coin.conf";
const char * const BITCOIN_PID_FILENAME = "v8coind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "v8coin" is a composite category enabling all V8COIN-related debug output
                if(ptrCategory->count(std::string("v8coin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\V8COIN
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\V8COIN
    // Mac: ~/Library/Application Support/V8COIN
    // Unix: ~/.v8coin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "V8COIN";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/V8COIN";
#else
    // Unix
    return pathRet / ".v8coin";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty v8coin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override v8coin.conf
