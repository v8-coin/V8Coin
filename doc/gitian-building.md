...
*Setup instructions for a Gitian build of V8COIN Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the V8COIN
...
- [Building V8COIN Core](#building-v8coin-core)
...
Clone the git repositories for V8COIN Core and Gitian.
...
in the V8COIN Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building V8COIN Core
----------------

To build V8COIN Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the V8COIN Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/v8coin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/v8coinpay/v8coin
...
For example:
```bash
URL=https://github.com/crowning-/v8coin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit v8coin=${COMMIT} --url v8coin=${URL} ../v8coin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit v8coin=${COMMIT} --url v8coin=${URL} ../v8coin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit v8coin=${COMMIT} --url v8coin=${URL} ../v8coin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the v8coin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../v8coin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/v8coin
SIGPATH=/some/root/path/v8coin-detached-sigs

./bin/gbuild --url v8coin=${BTCPATH},signature=${SIGPATH} ../v8coin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/v8coin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/v8coin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/v8coin-osx-build.assert
```
...
[v8coin/gitian.sigs](https://github.com/v8coinpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
