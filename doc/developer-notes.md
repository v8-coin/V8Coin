...
V8COIN Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadRPCServer : Remote procedure call handler, listens on port 8417 for connections and services them.
...
However, in open source software such as V8COIN Core, where everyone uses
...
pay attention to for reviewers of V8COIN Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: V8COIN Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against V8COIN Core so that they can be integrated
