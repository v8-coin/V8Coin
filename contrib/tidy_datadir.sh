...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete V8COIN database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no V8COIN datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old V8COIN datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected V8COIN 0.7 datadir."
    ;;
  3)
    echo "Detected V8COIN pre-0.8 datadir."
    ;;
  4)
    echo "Detected V8COIN 0.8 datadir."
    ;;
esac
